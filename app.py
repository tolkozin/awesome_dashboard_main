...
with st.sidebar.expander("General Settings", expanded=True):
    total_months = st.number_input("Projection Horizon (months)", min_value=12, max_value=120, value=60,
        help="Горизонт моделирования. По умолчанию 60 месяцев (5 лет).")
    phase1_dur = st.number_input("Phase 1 Duration (months)", min_value=1, max_value=48, value=3,
        help="Длительность фазы Pre-MVP — разработка, нет рекламы и пользователей.")
    phase2_dur = st.number_input("Phase 2 Duration (months)", min_value=1, max_value=48, value=3,
...
with st.sidebar.expander("Scenario Sensitivity"):
    ...
    sens_conv = st.slider("Conversion Sensitivity %", -50, 50, 0,
        help="Корректировка конверсий. + = лучше конверсии, - = хуже.")
    sens_churn = st.slider("Churn Sensitivity %", -50, 50, 0,
        help="Корректировка оттока. + = больше отток (хуже), - = меньше (лучше).")
    sens_cpi = st.slider("CPI Sensitivity %", -50, 50, 0,
        help="Корректировка CPI. + = дороже установки, - = дешевле.")
    sens_organic = st.slider("Organic Sensitivity %", -50, 50, 0,
        help="Корректировка органического роста. + = больше органики.")
    scenario_bound = st.slider("Scenario Bound %", 5, 50, 20,
        help="Границы пессимистичного/оптимистичного сценариев. Применяются ко всем переменным.")
    st.markdown("---")
    mc_enabled = st.checkbox("Monte Carlo Simulation", value=False,
...
with st.sidebar.expander("Trial, Refunds & COGS"):
    ...
    cogs_global = st.number_input("COGS per Active User ($)", min_value=0.0, value=0.10,
...
with st.sidebar.expander("Expansion & Contraction"):
    upgrade_rate = st.number_input("Monthly→Annual Upgrade %/mo", min_value=0.0, max_value=20.0, value=2.0,
        help="Процент месячных подписчиков, переходящих на годовой план каждый месяц.")
    downgrade_rate = st.number_input("Annual→Monthly Downgrade %/yr", min_value=0.0, max_value=50.0, value=5.0,
